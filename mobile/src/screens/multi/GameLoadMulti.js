...
const GameLoadMulti = () => {
  ...
  const { gameId, implementationGameBody, initGameModes, isCorrectAnswer,setIsCorrectAnswer} = useContext(SocketContext); // Se obtiene el id del juego desde el contexto
  // Referencia para guestWins // Contador de victorias del guest
...
const final = () => {
  ...
  navigation.reset({
    index: 0, // Índice de la pila de navegación
    routes: [{ name: 'GameFinishedMulti', params: { gameId } }] // Ruta a la que se navegará
  });
};
...
useEffect(() => {
  ...
  if (implementationGameBody) {
    // Asegúrate de que solo se ejecute una vez por ronda
    if (implementationGameBody.status === "FINISH_ROUND" && !roundFinished) {
      let message = "";
...
}, [implementationGameBody]);  // Dependencias del useEffect
  ...
  const handleCorrectAnswer = () => {
    ...
    if (nextIndex >= gameKeys.length) {
      ...
      // Marca la ronda como terminada
      setRoundFinished(false); // Si no es la última fase, avanza al siguiente índice
    }
  };
