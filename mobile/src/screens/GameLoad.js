...
const GameLoad = () => {
  ...
  const sAD = async (idGameSingle, idUser, text, gameModeId, time) => {
    try {
      await sendAnswer(idGameSingle, idUser, text, gameModeId, time);
    } catch (error) {
  ...
  useEffect(() => {  
    if(gameFinished){
      const timer = setInterval(() => {
        if (!isCheckingAnswer) { // Solo ejecutar si el componente está montado
          setTimeLeft(prev => {
            if (prev <= 1) {
              setCurrentGameIndex(prevIndex => {
                const nextIndex = prevIndex + 1;
                console.log("Cantidad de modos de juego: " + Object.keys(data).length);
                ...
                if (nextIndex >= Object.keys(data).length) {
                    ...
                    sAD(idGameSingle, 0, "", id, 30);
                    console.log("Esta entrando aca luego de lo que deberia");
                    navigation.reset({
                    index: 0,
                    routes: [
                      {
    ...
    setCurrentGameIndex(prevIndex => {
      ...
      if (nextIndex >= Object.keys(data).length) {
        if(gameFinished){
          ...
          }); // Navegar a pantalla de resumen de partida
